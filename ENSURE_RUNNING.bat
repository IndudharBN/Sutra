...
if errorlevel 1 (
  ...
  PowerShell -NoProfile -Command "$p = (Get-NetTCPConnection -LocalPort 3001 -ErrorAction SilentlyContinue | Where-Object { $_.OwningProcess -gt 4 } | Select-Object -First 1).OwningProcess; if ($p) { Stop-Process -Id $p -Force; Start-Sleep -Seconds 1; Write-Host '[ENSURE] Killed stale PID' $p }"
