...
// the timeframe actually traded. A daily-ATR stop sat a full-day move away (~3.8% of
// price median), so intraday winners were flattened at EOD before reaching target
// (realized payoff 0.89). Re-anchored to 15m 2026-08-27.
//
// BUG FIXED 2026-09-10: this function used Math.min/Math.max as a FLOOR (widen a
// too-tight structural stop). That logic was written for DAILY ATR (wide). With a
// NARROW 15m ATR it did the opposite of the intent: min(wideStructural, narrowFloor)
// on a BULL kept the WIDER structural stop and discarded the tight 15m level, so the
// median stop stayed 4.5% and realized payoff never moved (1.00→1.03). Now it CAPS:
// the 15m-ATR distance is a CEILING on stop width. If the structural stop is wider
// than the cap, pull it IN to the cap (tighter wins); if it is already tighter,
// keep it. This is what actually anchors the stop to the intraday timeframe.
//
// CALIBRATION: a single 15m bar's TR is ~1/10th of daily ATR, so 15mATR × the old
// 0.75 noiseFloor = ~0.3% stop — too tight, wicked out. INTRADAY_ATR_MULT scales the
// cap to a sane width: net ≈ 15mATR × 3 at base VIX → cap ~1.2% of price (p25 0.95%,
// p75 1.79%). Tight enough to ~triple realized R:R, wide enough to survive 5m noise.
const INTRADAY_ATR_MULT = 4;
function intradayCappedStop(direction: 'BULL' | 'BEAR', entry: number, rawStop: number, atrIntraday: number, vixLevel?: number | null): number {
  const nf = noiseFloor(vixLevel) * INTRADAY_ATR_MULT;
  const capDist = atrIntraday * nf;
  if (!(capDist > 0)) return rawStop; // no valid 15m ATR — fall back to structural stop
  const cap = direction === 'BULL' ? entry - capDist : entry + capDist;
  // Tighter of the two wins: for BULL the higher stop price is tighter (closer to entry).
  return direction === 'BULL' ? Math.max(rawStop, cap) : Math.min(rawStop, cap);
}
...
export function evaluateOrbRetest(input: StrategyInput): StrategySignal {
  ...
  const stop = enforceMinStop(dir, entry, intradayCappedStop(dir, entry, rawStop, (input.atr15 ?? input.atr20 * 0.3), input.vixLevel), (input.atr15 ?? input.atr20 * 0.3));
...
export function evaluateObFvgRetest(input: StrategyInput): StrategySignal {
  ...
  const stop = enforceMinStop(dir, entry, intradayCappedStop(dir, entry, cappedStop, (input.atr15 ?? input.atr20 * 0.3), input.vixLevel), (input.atr15 ?? input.atr20 * 0.3));
  const risk = Math.abs(entry - stop);
  const t1 = dir === 'BULL' ? entry + risk * T1_RR : entry - risk * T1_RR;
  const t2 = structuralT2(selfInput, entry, risk, t1);
...
export function evaluateMssBreakout(input: StrategyInput): StrategySignal {
  ...
  const stop = enforceMinStop(dir, entry, intradayCappedStop(dir, entry, swingStop, (input.atr15 ?? input.atr20 * 0.3), input.vixLevel), (input.atr15 ?? input.atr20 * 0.3));
  const risk = Math.abs(entry - stop);
  const t1 = dir === 'BULL' ? entry + risk * T1_RR : entry - risk * T1_RR;
  const t2 = structuralT2(selfInput, entry, risk, t1);
  const planCandidate = planFromLevelsT1T2(selfInput, entry, stop, t1, t2, trigger);
...
function checkS7VolumeSurge(input: StrategyInput): StrategySignal | null {
  ...
  const stop = enforceMinStop(dir, price, intradayCappedStop(dir, price, rawStop, (atr15 ?? input.atr20 * 0.3), input.vixLevel), (atr15 ?? input.atr20 * 0.3));
...
export function evaluateOrb15mRetest(input: StrategyInput): StrategySignal {
  ...
  const rawStop = ob && atOb
    ...
    : entry;
  const stop = intradayCappedStop(dir, entry, rawStop, (input.atr15 ?? input.atr20 * 0.3), input.vixLevel);
  const risk = Math.abs(entry - stop);
  const t1 = dir === 'BULL' ? entry + risk * T1_RR : entry - risk * T1_RR;
  const t2 = structuralT2(selfInput, entry, risk, t1);
...
export function evaluateSniper1m(input: StrategyInput): StrategySignal {
  ...
  const rawStop = ob1m && atOb1m
    ...
    : entry;
  // Sniper-scale stop floor — deliberately NOT the book-wide intradayCappedStop. That
